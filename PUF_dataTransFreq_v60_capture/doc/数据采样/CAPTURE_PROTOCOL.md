...
**换传感器前**：跑 `python scripts/check_v66_stability.py logs/<dataset> --sensor <id>` 全 OK 再走。
**采完整批**：跑 `check_v66_stability.py logs/<dataset>` 全量扫描 + `post_process.py --glob` 生成 `all_dataset.npz` + `manifest.json`，确认 `total_valid == total_samples`。
