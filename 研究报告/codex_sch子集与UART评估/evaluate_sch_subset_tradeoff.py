...
def read_file(csv_path: Path) -> dict[int, dict[str, np.ndarray]]:
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as fh:
        for row in csv.DictReader(fh):
            ...
            sch_map.setdefault(sch, {})[line_type] = vec
    return sch_map
...
def feature_from_subset(sch_map: dict[int, dict[str, np.ndarray]], subset: set[int]) -> np.ndarray:
    ...
    for line_type in LINE_TYPES:
        arr = np.vstack(collected[line_type]) if collected[line_type] else np.zeros((1, 128), dtype=np.float64)
...
def load_cache() -> tuple[list[dict[int, dict[str, np.ndarray]]], np.ndarray, list[str]]:
    cached = []
    labels = []
    files = []
    ...
    return cached, np.array(labels), files
...
def rank_single_sch(cached: list[dict[int, dict[str, np.ndarray]]], y: np.ndarray) -> list[dict[str, float | int]]:
    ...
    for sch in range(256):
        ...
        ranked.append(
            {
                "sch": sch,
                "cv_acc_proxy": centroid_acc,
                "silhouette": sil,
                "centroid_acc": centroid_acc,
            }
        )
    ranked.sort(key=lambda item: (item["cv_acc_proxy"], item["silhouette"], item["centroid_acc"]), reverse=True)
    return ranked
...
def evaluate_top_subsets(cached: list[dict[int, dict[str, np.ndarray]]], y: np.ndarray, ranked_sch: list[dict[str, float | int]]) -> list[dict[str, object]]:
    ...
    for top_n in TOP_COUNTS:
        ...
        binary_bytes_per_record = 512 + 16
...
def main() -> None:
    cached, labels, files = load_cache()
